...
from structures.bbox import get_udp_warp_matrix, get_warp_matrix
...
class TopdownAffine:
    ...
    def __init__(self, input_size: Tuple[int, int], use_udp: bool = False) -> None:
        assert isinstance(input_size, tuple) and len(input_size) == 2 and isinstance(input_size[0],int), \
            f'Invalid input_size {input_size}, must be a Tuple[int, int]'

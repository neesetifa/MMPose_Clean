...
from structures.bbox import bbox_xyxy2cs, flip_bbox
from structures.keypoint import flip_keypoints
...
class RandomFlip:
    ...
    def __init__(self,
        ...
        if isinstance(prob, list):
            assert isinstance(prob, list) and isinstance(prob[0], float)
            assert 0 <= sum(prob) <= 1
    ...
    def transform(self, results: dict) -> dict:
        ...
        if flip_dir is None:
            ...
            if results.get('bbox', None) is not None:
                results['bbox'] = flip_bbox(results['bbox'],
                                            image_size=(w, h),
                                            bbox_format='xyxy',
                                            direction=flip_dir)
                
            if results.get('bbox_center', None) is not None:
                results['bbox_center'] = flip_bbox(results['bbox_center'],
                                                   image_size=(w, h),
                                                   bbox_format='center',
                                                   direction=flip_dir)
...
class GenerateTarget:
    ...
    def __init__(self,
        ...
        if isinstance(self.encoder_cfg, list):
            self.encoder = []
            for cfg in self.encoder_cfg:
                codec_class = eval(cfg['type'])
                del cfg['type']
                tmp = codec_class(**cfg)
                self.encoder.append(tmp)
        else:
            assert not self.multilevel, ('Need multiple encoder configs if ``multilevel==True``')
            codec_class = eval(self.encoder_cfg['type'])
            del self.encoder_cfg['type']
            self.encoder = codec_class(**self.encoder_cfg)
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        if not isinstance(self.encoder, list):
            ...
            encoded = self.encoder.encode(keypoints=keypoints,

...
class BaseModel(BaseModule):
    ...
    def train_step(self, data: Union[dict, tuple, list],
        ...
        with optim_wrapper.optim_context(self):
            # 此函数在同目录data_preprocessor.py的ImgDataPreprocessor,
            # 这里data已经对图像和对应的label做完数据增强, 完成统一输入shape的工作, 此函数仅对输入图像做简单normalize处理
            data = self.data_preprocessor(data, True)
            # 此函数就在本文件, 是一个抽象函数, 没有实际功能
            # 具体forward()实现在子类里, 比如mmpose/models/pose_estimators/base.py 的 BasePoseEstimator
            # 在forward里调用子类的 predict() 或者 loss(), 此类函数可以查看mmpose/models/heads/ 里各个head函数
            losses = self._run_forward(data, mode='loss')  # type: ignore
        parsed_losses, log_vars = self.parse_losses(losses)  # type: ignore 此函数就在本文件

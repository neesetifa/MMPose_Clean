...
from structures.bbox import bbox_xyxy2xywh
...
class CocoMetric(BaseMetric):
    ...
    def dataset_meta(self, dataset_meta: dict) -> None:
        ...
        if self.gt_converter is not None:
            dataset_meta['sigmas'] = transform_sigmas(dataset_meta['sigmas'],
    ...
    def process(self, data_batch: Sequence[dict], data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            if isinstance(data_sample, BaseDataElement):
                data_sample = data_sample.to_dict() # convert BaseDataElement to Dict
    ...
    def gt_to_coco_json(self, gt_dicts: Sequence[dict], outfile_prefix: str) -> str:
        ...
        for gt_dict in gt_dicts:
            ...
            if gt_dict['img_id'] not in img_ids:
                image_info = dict(id=gt_dict['img_id'],
                                  width=gt_dict['width'],
        ...
        with open(converted_json_path, 'w') as f:
            json_obj = json.dumps(coco_json, sort_keys=True, indent=4)
            f.write(json_obj)
    ...
    def compute_metrics(self, results: list = None) -> Dict[str, float]:
        ...
        if self.gt_converter is not None:
            for id_, ann in self.coco.anns.items():
                self.coco.anns[id_] = transform_ann(ann,
                                                    self.gt_converter['num_keypoints'],
                                                    self.gt_converter['mapping'])
        ...
        for img_id, instances in kpts.items():
            for instance in instances:
                ...
                instance['keypoints'] = np.concatenate([instance['keypoints'],
                                                        instance['keypoint_scores'][:, None]],
                                                       axis=-1)
        ...
        if self.format_only:
            print(f'results are saved in {osp.dirname(outfile_prefix)}')
            return {}
    ...
    def results2json(self, keypoints: Dict[int, list],
        ...
        with open(res_file, 'w') as f:
            json_obj = json.dumps(cat_results, sort_keys=True, indent=4)
            f.write(json_obj)

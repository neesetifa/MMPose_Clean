...
class PCKAccuracy:
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        if 'bbox' in self.norm_item:
            ...
            print(f'Evaluating {self.__class__.__name__} (normalized by ``"bbox_size"``)...')
        ...
        if 'head' in self.norm_item:
            ...
            print(f'Evaluating {self.__class__.__name__} (normalized by ``"head_size"``)...')
        ...
        if 'torso' in self.norm_item:
            ...
            print(f'Evaluating {self.__class__.__name__} (normalized by ``"torso_size"``)...')
...
class NME:
    ...
    def _get_normalize_factor(self, gt_coords: np.ndarray) -> np.ndarray:
        ...
        interocular = np.linalg.norm(gt_coords[:, idx1, :] - gt_coords[:, idx2, :],
                                     axis=1,
                                     keepdims=True)

...
from .utils.refinement import refine_simcc_dark
from .base import BaseKeypointCodec


class SimCCRegLabel(BaseKeypointCodec):
    r"""Generate keypoint representation via "SimCC" approach and Regression Label
    ...
    """
    label_mapping_table = dict(keypoint_x_labels='keypoint_x_labels',
    ...
    def encode(self,
        ...
        # 1. Generate Simcc Label
        if self.smoothing_type == 'gaussian':
            x_labels_simcc, y_labels_simcc, keypoint_weights = self._generate_gaussian(
                keypoints, keypoints_visible)
        elif self.smoothing_type == 'standard':
            x_labels_simcc, y_labels_simcc, keypoint_weights = self._generate_standard(
                keypoints, keypoints_visible)
        else:
            raise ValueError(
                f'{self.__class__.__name__} got invalid `smoothing_type` value'
        ...
        w, h = self.input_size
        keypoint_labels_regression = (keypoints / np.array([w, h])).astype(np.float32)

        # 字典名称在上面重新定义.
        # 这样在mmpose/datasets/transforms/formatting.py 会更新 label_mapping_table
        # 否则如果使用其他的名字, 则不会在 formatting 里被打包
        encoded = dict(keypoint_x_labels=x_labels_simcc,
                       keypoint_y_labels=y_labels_simcc,
                       keypoint_labels=keypoint_labels_regression,
                       keypoint_weights=keypoint_weights)
    ...
    def decode(self,
               simcc_x: np.ndarray,
               simcc_y: np.ndarray,
               pred_rle: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Args:
            encoded (Tuple[np.ndarray, np.ndarray]): SimCC labels for x-axis
                and y-axis
            simcc_x (np.ndarray): SimCC label for x-axis, [batch, 17, Wx]
            simcc_y (np.ndarray): SimCC label for y-axis, [batch, 17, Wy] 
        ...
        # 1. Decode using SIMCC head
        """
        x_locs = np.argmax(simcc_x, axis=1)  # [batch*17, Wx]
        y_locs = np.argmax(simcc_y, axis=1)  # [batch*17, Wy]
        locs = np.stack((x_locs, y_locs), axis=-1).astype(np.float32)
        max_val_x = np.amax(simcc_x, axis=1)
        max_val_y = np.amax(simcc_y, axis=1)
        
        mask = max_val_x > max_val_y      # x和y两个confidence, 取"较小"的那个值
        max_val_x[mask] = max_val_y[mask]
        vals = max_val_x
        locs[vals <= 0.] = -1             # 并把分数小于0的坐标值设置为-1
        ...
        # 2. Decode using RLE head
        # normalized_coords = pred_rle[..., :2].copy()
        # output_sigma = pred_rle[..., 2:4].copy()
    ...
    def _generate_gaussian(
        ...
        N, K, _ = keypoints.shape # [batch, 17, 2]
        w, h = self.input_size
        W = np.around(w * self.simcc_split_ratio).astype(int)  # input_w*2 = 192*2=384
        H = np.around(h * self.simcc_split_ratio).astype(int)  # input_h*2 = 256*2=512

        keypoints_split, keypoint_weights = self._map_coordinates(
            keypoints, keypoints_visible) # [batch, 17, 2], [batch, 17]
        ...
        if self.normalize:
            # 默认做normalize
            # 由于 target_xy 值域范围为 [0,1]
            # norm_value = 6*sqrt(2*pi) = 15.039769647786002
            # 所以normalize之后 target_xy 值域为 [0,1]/15.039769647786002 = [0, 0.06649]

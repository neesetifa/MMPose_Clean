...
from ...utils.tta import flip_vectors
from ...loss import build_loss
from codec.utils import get_simcc_normalized
from codec import build_codec
from evaluation.functional import simcc_pck_accuracy
...
class SimCCHead(BaseHead):
    ...
    def __init__(self,
                 ...
                 loss: dict = dict(type='KLDiscretLoss', use_target_weight=True),
                 decoder = None,
                 init_cfg = None,
                 ):
    ...
    def predict(self, feats: Tuple[Tensor], batch_data_samples, test_cfg = {}):
        ...
        if test_cfg.get('output_heatmaps', False):
            ...
            pred_fields = [hm for hm in batch_heatmaps.detach()]

            pred_x_np = batch_pred_x.detach().cpu().numpy()
            pred_y_np = batch_pred_y.detach().cpu().numpy()
            for pred_instances, pred_x, pred_y in zip(preds, pred_x_np, pred_y_np):
    ...
    def loss(self, feats: Union[Tensor, Tuple[Tensor]], batch_data_samples, train_cfg = {}) -> dict:
        ...
        _, avg_acc, _ = simcc_pck_accuracy(output=pred_simcc_np,
                                           target=gt_simcc_np,

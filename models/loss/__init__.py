...
def build_loss(cfg):
    ...
    loss_class = eval(loss_cfg.pop('type'))
...
class CombinedLoss(nn.ModuleDict):
    ...
    def __init__(self, losses: Dict, losses_weight: Dict):
        super().__init__()
        for loss_name, loss_cfg in losses.items():
            t_loss = build_loss(loss_cfg)
            self.add_module(loss_name, t_loss)
        for loss_weight_name, loss_weight in losses_weight.items():
...
__all__ = [
    ...
    'KLDiscretLoss', 'JSDiscretLoss', 'SoftWeightSmoothL1Loss',
    'CombinedLoss','MultipleLossWrapper', 'build_loss'
]

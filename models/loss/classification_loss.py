...
class KLDiscretLoss(nn.Module):
    ...
    def criterion(self, dec_outs, labels):
        """Criterion function."""
        # dec_outs shape = [batch*17, input_size * simcc_ratio]
        # labels shape = [batch*17, input_size * simcc_ratio]
        log_pt = self.log_softmax(dec_outs * self.beta)
        if self.label_softmax:  # 默认False
            labels = F.softmax(labels * self.beta, dim=1)
        loss = torch.mean(self.kl_loss(log_pt, labels), dim=1) # shape=[batch*17, ]
        return loss
    ...
    def forward(self, pred_simcc, gt_simcc, target_weight):
        ...
        if self.use_target_weight:
            weight = target_weight.reshape(-1)  # [batch, 17] -> [batch*17,]
        else:
            weight = 1.

        # pred_simcc[0]/[1] shape = [batch, 17, 192*2 = input_size * simcc_ratio]
        # gt_simcc[0]/[1]   shape = [batch, 17, 192*2 = input_size * simcc_ratio]
        for pred, target in zip(pred_simcc, gt_simcc):
            # [batch, 17, input_size * simcc_ratio] -> [batch*17, input_size * simcc_ratio]
